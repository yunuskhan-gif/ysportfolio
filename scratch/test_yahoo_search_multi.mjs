...
async function search(query) {
  try {
    ...
    if (res.data.quotes && res.data.quotes.length > 0) {
      ...
      res.data.quotes.slice(0, 3).forEach(q => {
        console.log(`- Symbol: ${q.symbol}, Name: ${q.shortname || q.longname}, Exchange: ${q.exchDisp || q.exchange}`);
      });
    }
  } catch (err) {
...
async function run() {
  await search("SBI Bluechip");
  await search("TCS");
  await search("Reliance");
}
run();
